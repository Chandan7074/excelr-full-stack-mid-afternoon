...
constructor execution. somehow run time enviroment creating the
new object from the original copied content. if we try
...
attributes to that newly created object, instead of this lengthy
...
object class clone method can not be used. so in case of cloning
security issues for ex: if we have a two keys to the lock for our
home we need to keep both key safely
*/
